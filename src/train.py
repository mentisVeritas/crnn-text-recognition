...
from src.utils import get_device, project_root_from_checkpoint_path, setup_outputs_file_logging

logger = logging.getLogger(__name__)


def collate_fn_ctc(batch):
    """Batch dict samples into tensors/lists expected by CTC training loop."""
...
def build_dataloaders(
    ...
    """Build train/val split and corresponding dataloaders from OCR dataset."""
    dataset = OCRDataset(
...
def _filter_batch_for_ctc(images, labels_list, lengths, max_input_len, target_device):
    """Drop samples that violate CTC length constraints for current model width."""
...
def run_epoch(
    ...
    """Run one train/val epoch with CTC safety checks and optional tqdm."""
    model.train(train)
    total_loss = 0.0
    valid_steps = 0
    skipped_samples = 0
    ...
    for batch in iterator:
        ...
        # CTCLoss on MPS can be unstable; move tensors to CPU for the loss call.
        if device.type == "mps":
...
def load_weights_for_inference(
    ...
    """Load best checkpoint (fallback: last checkpoint) and switch model to eval mode."""
...
def run_training(config, project_root=None, split_seed: int = 42):
    ...
    # CLI policy: each run starts from fresh weights, then updates checkpoints.
