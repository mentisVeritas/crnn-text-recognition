...
def _make_predict_fn(model: torch.nn.Module, alphabet: str, device: torch.device) -> Callable[
    [torch.Tensor], tuple[str, float]]:
...
def visualize_val_predictions(
    ...
    """Показывает изображения в одну колонку (1 строка = 1 изображение) + метрики."""
    ...
    fig, axes = plt.subplots(len(rows), 1, figsize=(18, 2 * len(rows)))
...
def show_hard_val_examples(
    ...
    """Показывает самые сложные ошибки: 1 строка = 1 изображение."""
    ...
    # сортировка по edit distance
    rows_sorted = sorted(
        rows,
        key=lambda x: (x["dist"], -x["confidence"]),
        reverse=True,
    )
    ...
    fig, axes = plt.subplots(
        len(hard),
        1,
        figsize=(16, 3 * len(hard)),
    )

    if len(hard) == 1:
        axes = [axes]

    for ax, row in zip(axes, hard):
        image = row["image"]
    ...
    logger.info(
        "Analyzed samples: %s | mistakes: %s/%s",
        len(rows),
        mistakes,
        len(rows),
    )
